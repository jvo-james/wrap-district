...
function getOptionByValue(group, value) {
...
if (productId === "promo-coke") return 0;
...
for (const group of product.optionGroups) {
  if (group.type === "radio") {
    ...
    const selectedOption =
      getOptionByValue(group, selectedValue) ||
      group.options.find((opt) => opt.default) ||
      group.options[0];

    price += Number(selectedOption?.price) || 0;
  } else if (group.type === "checkbox") {
    const selectedValues = Array.isArray(selections[group.key]) ? selections[group.key] : [];
    for (const value of selectedValues) {
      const option = getOptionByValue(group, value);
      price += Number(option?.price) || 0;
    }
  } else {
    const selectedValue = selections[group.key];
    const selectedOption = getOptionByValue(group, selectedValue) || group.options?.[0];
    price += Number(selectedOption?.price) || 0;
  }
}
...
wrap.innerHTML = <i class="fa-solid fa-bag-shopping"></i> <p>${esc(text)}</p>;
return wrap;
}
...
btn.innerHTML = <i class="fa-regular fa-folder-open"></i> <span>No purchases found yet.</span>;
return btn;
}
...
btn.innerHTML = <div class="history-item__main"> <strong>${esc(order.reference)}</strong> <span>${esc(order.name)}</span> </div> <div class="history-item__meta"> <span>${esc(order.phone)}</span> <strong>${esc(money(order.total))}</strong> </div>;
