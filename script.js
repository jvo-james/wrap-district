(() => {
  ...
  function getOptionPriceByValue(product, groupKey, selectedValue) {
    const group = (product.optionGroups || []).find((g) => g.key === groupKey);
    if (!group) return 0;
  ...
  function computeUnitPrice(productId, selections) {
    ...
    if (productId === "loaded-angwamo") {
      return getOptionPriceByValue(product, "tier", selections.tier || "Basic");
    }

    if (productId === "shawarma") {
      const sizePrice = getOptionPriceByValue(product, "size", selections.size || "Lite");
      const proteinPrice = getOptionPriceByValue(product, "protein", selections.protein || "None");
      const extras = Array.isArray(selections.extras) ? selections.extras : [];
      const extrasPrice = extras.reduce((sum, value) => {
        return sum + getOptionPriceByValue(product, "extras", value);
      }, 0);
      return sizePrice + proteinPrice + extrasPrice;
    }

    if (productId === "loaded-fries") {
      const portionPrice = getOptionPriceByValue(product, "portion", selections.portion || "Regular");
      const toppings = Array.isArray(selections.toppings) ? selections.toppings : [];
      const toppingsPrice = toppings.reduce((sum, value) => {
        return sum + getOptionPriceByValue(product, "toppings", value);
      }, 0);
      return portionPrice + toppingsPrice;
    }

    if (productId === "fried-rice") {
      // FIX: price should follow the selected protein option, not stay at the default.
      return getOptionPriceByValue(product, "protein", selections.protein || "Regular");
    }
